...
%%
%% Components
KAOWOOL = char(MATERIALS{:,:,1});
Aluminum = char(MATERIALS{:,:,2});
PMMA = char(MATERIALS{:,:,3}); 
HIPS = char(MATERIALS{:,:,4});
Kydex = char(MATERIALS{:,:,5});
Cardboard = char(MATERIALS{:,:,6});
PEI = char(MATERIALS{:,:,7});
PET = char(MATERIALS{:,:,8});
POM = char(MATERIALS{:,:,9});
ABS = char(MATERIALS{:,:,10});
...
% MATERIALS = get(Materials,'Value');
% index_selected = get(hObject,'Value'); %find out exactly what the function is doing by testing up to this point.
% %[index_selected] = get(hObject,'Value');
% NI = size(index_selected,2); %=2 No idea what size does. Throwing me off
% for j = 1:NI %Thermodynamics (most likely mean from variable I to Variable NI)
%    a{j}=char(MATERIALS{:,1,I(j)});
% end
% b=char(MATERIALS{:,1,11}); %Mixtures %understandable
% for j = 1:NI %Kinematics Understand that this is the cause for it too loop but not 
%     c(j)=char(MATERIALS{:,2,I(j)}); %same reason
% end
% RMAT = char(a(:),b,c(:)); %trying to get all values in for loops to variable so as write it in .cmp file.
% 
% FileName = uiputfile('*.cmp','Save Component file as');
% dlmwrite(FileName,RMAT,'\t');
index_selected = get(hObject,'Value');

Materials = {ABS,Aluminum,Cardboard,HIPS,KAOWOOL,Kydex,PEI,PET,PMMA,POM};
for i =1:size(index_selected,2)
    RMAT(i)=(Materials(index_selected(i)));
end
