...
file_strings_sz = size(file_strings{1}); %size of file_strings (number of rows)
file_strings_ix = 0; %reset counter for amount of lines
cmp_ix = 0; %resets counter for amount of components
state_ix = 0;
while file_strings_ix < file_strings_sz(1,1) %while the counter is less than the actual amount of lines in file continue loop
    file_strings_ix = file_strings_ix+1; % add 1 to the counter to identify what line it is on
    if strcmp(file_strings{1}{file_strings_ix},'COMPONENT') % if loop reaches string 'COMPONENT'...
...
F = figure('Visible','on','Position',[680,280,270,170]...
    ...
    remlayer = uicontrol('Parent',L,'Style','pushbutton','String','-',...
        ...
        'FontSize',20,'FontWeight','bold','Visible','Off');
  ...
  %Create scrollbar for moving content in figure
     vscrollos = uicontrol('Parent',F,'Style','slider',...
        'Units','pixels','Position',[255 0 15 170],'Tag','v_scrollos','Visible','Off');
...
disp(Lcounter.String);
positionX = [5 35 210 120];

    h = uipanel('Parent',L,'Tag',sprintf('oslayer_%s',Lcounter.String),...
        'Units','pixels','Position',positionX,...
...
%         'Tag',sprintf('text2_%d',int32(uicontrol_ix)),'Visible','on');
%     
...
%         'Tag',sprintf('Thick_%d',int32(uicontrol_ix)),'Visible','on');
%     
...
function add_mf_X_Callback(hObject, eventdata, handles)
function rem_mf_X_Callback(hObject, eventdata, handles)
